...
inquirer
    ...
    .then(function (answers) {
        generatorMiddleware([
            // 根据条件获取工程目录路径
            function getProDirPath (next, state) {
                // 获取当前目录的所有子目录和文件
                ...
                if (files.length) {
                    ...
                    if (files.filter(function (file) {
                        return fs.statSync(file).isDirectory() && file === proName;
                    }).length) {
                        console.error(chalk.red(state.proName + '已经存在，请重新创建'));
                        return;
                    }
                } else if (currDirName === proName) {
                    proName = '.'
                } 

                next(Object.assign(state, { 
                    proDirPath: Object__wrapperPath(proName) 
                }));    
            },

            // 创建工程目录
            function createProDir (next, state) {
                fs.mkdir(state.proDirPath, function (err) {
                    if (err) {
                        console.error(chalk.red('创建' + proDirPath + '失败，请重新创建'));
                        return;
                    }
            ...
            function updateProTemp (next, state) {
                // 获得对应的文件
                var packageJSONFilePath = path.join(state.dest, 'package.json'),
                    indexViewFilePath = path.join(state.dest, 'index.html');
